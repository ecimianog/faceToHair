...
drawing_specC = mp_drawing.DrawingSpec(color=(255, 255, 255), thickness=1, circle_radius=1)
drawing_specD = mp_drawing.DrawingSpec(color=(255, 100, 0), thickness=3, circle_radius=1)
...
def lookFront(image):
    ...
    if face.multi_face_landmarks:
        for face_landmarks in face.multi_face_landmarks:
            ...
            lips_landmarksB.landmark.extend([face_landmarks.landmark[id] for id in LIPS_LANDMARKS])
            mp_drawing.draw_landmarks(
                    image=image,
            ...
            eye_landmarksC.landmark.extend([face_landmarks.landmark[id] for id in RIGHT_EYE_LANDMARKS])
            eye_landmarksC.landmark.extend([face_landmarks.landmark[id] for id in LEFT_EYE_LANDMARKS])
            mp_drawing.draw_landmarks(
                    image=image,
            ...
            oval_landmarksD.landmark.extend([face_landmarks.landmark[id] for id in FACE_OVAL_LANDMARKS])
            mp_drawing.draw_landmarks(
                    image=image,
            ...
            for idx, lm in enumerate(face_landmarks.landmark):
                ...
                if idx not in LEFT_EYEBROW_LANDMARKS and idx not in RIGHT_EYEBROW_LANDMARKS and idx not in LIPS_LANDMARKS and idx not in LEFT_EYE_LANDMARKS and idx not in LEFT_IRIS_LANDMARKS and idx not in RIGHT_EYE_LANDMARKS and idx not in RIGHT_IRIS_LANDMARKS and idx not in FACE_OVAL_LANDMARKS:    
                    rest_landmarks.landmark.extend([face_landmarks.landmark[idx]])
